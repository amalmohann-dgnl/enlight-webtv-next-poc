...
const { getConnectionType, getDeviceModel, isAndroid, getDeviceCategory, getFormattedPlatformVersion } = deviceUtilities;
...
const {
    VITE_LOGGLY_API_KEY: LOGGLY_API_KEY,
    VITE_LOGGLY_BASE_URL: LOGGLY_BASE_URL,
...
class LogglyServices {
    ...
    async sendLog(errorInfo: SendLogParameterType) {
        ...
        const stableAttributes = await this.initializeStableLogAttributes(appName, appVersion);
        const authAttributes = await this.initializeAuthAttributes();
        const extraAttributes = this.initializeExtraAttributes(errorInfo.errorPathObject, errorInfo?.eventType ?? '');
        ...
        switch (errorInfo.module) {
            case ModuleType.Search:
                errorData = await this.constructSearchModuleData(errorInfo.logLevel, errorInfo.data as SearchModuleData, errorInfo.errorCode!);
                break;
            case ModuleType.Authentication:
                errorData = await this.constructAuthenticationModuleErrorData(errorInfo.logLevel, errorInfo.data, errorInfo.errorCode ?? '');
                break;
            case ModuleType.General:
                errorData = await this.constructGeneralModuleErrorData(errorInfo.logLevel, errorInfo.data, errorInfo.errorCode!);
                break;
            case ModuleType.Profile:
                errorData = await this.constructProfileModuleErrorData(errorInfo.logLevel, errorInfo.data, errorInfo.errorCode!);
                break;
            case ModuleType.Player:
                errorData = await this.constructPlayerModuleErrorData(errorInfo.data, errorInfo.errorCode, errorInfo.logLevel);
                break;
        }
        ...
        if (isValidValue(errorInfo?.apiErrorData)) {
            ...
            apiErrorData = {
                ...
                response_message: apiError?.response?.data?.errorMessage ?? apiError?.message,
        ...
        const logData = {
            ...
            ...(apiErrorData && {
                ...
                mw_error_code:
                    ...
                    errorInfo?.apiErrorData?.responseData?.response?.data?.error?.errorCode,
            }),
        };

        // send post request to loggly
        axios.post(`${LOGGLY_BASE_URL}/${LOGGLY_API_KEY}/tag/${appName}/`, logData, {
     ...
     */
    async constructPlayerModuleErrorData(data?: ErrorLogData, errorCode?: string, logLevel: LogLevel = LogLevel.WARNING) {
     ...
     * @author tonyaugustine
     */
    async constructAuthenticationModuleErrorData(logLevel: LogLevel, data: any, errorCode: string) {
        const errorData = await this.constructErrorLog(logLevel, data, errorCode);
        return errorData;
    }
     ...
     * @author gowripriyadileep
     */
    async constructSearchModuleData(logLevel: LogLevel, data: SearchModuleData, errorCode: string) {
        const errorData = await this.constructErrorLog(logLevel, data, errorCode);
        return { ...errorData };
    }
     ...
     * @author gowripriyadileep
     */
    async constructGeneralModuleErrorData(logLevel: LogLevel, data: any, errorCode: string) {
        const errorData = await this.constructErrorLog(logLevel, data, errorCode);
        return errorData;
    }
     ...
     * @author tonyaugustine
     */
    async constructProfileModuleErrorData(logLevel: LogLevel, data: any, errorCode: string) {
        const errorData = await this.constructErrorLog(logLevel, data, errorCode);
        return errorData;
    }
     ...
     * @author tonyaugustine
     */
    async constructErrorLog(logLevel: string = LogLevel.INFO, data?: ErrorLogData, errorCode?: string) {
        ...
        if (errorCode) {
            ...
            contentfulDataError = getErrorByCode(data?.responseData?.errorCode?.toString());
        }
        ...
        const UICode = this.getUICode(errorCode ?? data?.responseData?.errorCode?.toString() ?? '');
     ...
     * @author tonyaugustine
     */
    initializeExtraAttributes(errorObj: Error, eventType: string = '') {
        // generate the error location path
        const errorPath = generatePathFromErrorObject(errorObj);

        // get page related attributes
        const pageURL = window.location.href;
        return { timestamp: new Date().toISOString(), sourcelocation: errorPath?.slice(2).trim(), event: eventType, page_url: pageURL };
    }
    ...
    async initializeStableLogAttributes(appName: string, appVersion: string) {
        const platformValue = IS_ANDROID ? getDeviceData().deviceType : Platform.WebTv;
        ...
        const stableLogAttributes = {
            ...
            //eslint-disable-next-line @typescript-eslint/ban-ts-comment
            //@ts-ignore
            app_env: import.meta.env.VITE_BUILD_TYPE,
            browser: userAgent?.browserName,
            browser_version: userAgent?.browserVersion,
            device_brand: deviceInfo?.brand,
            device_model: getDeviceModel(),
            device_category: getDeviceCategory(),
            lang: (getState(StorageKeys.PROFILE) as ProfileListData)?.preferences?.language ?? 'en-US',
