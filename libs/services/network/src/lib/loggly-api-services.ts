...
import { logglyUtilities, userAgentUtilities, appUtilities, authUtilities, deviceUtilities, storageUtilities } from '@enlight-webtv/utilities';
...
export default class LogglyAPIServices {
    ...
    async sendLog(errorInfo: any) {
        ...
        axios.post(`${LOGGLY_BASE_URL}/${LOGGLY_API_KEY}/tag/${appName}/`, finalLog, {
    ...
    initializeExtraAttributes() {
        ...
        return { timestamp: new Date().toISOString(), event: null, page_url: pageURL };
    }
    ...
    async initializeStableLogAttributes(appName: string, appVersion: string) {
        //eslint-disable-next-line @typescript-eslint/ban-ts-comment
        //@ts-ignore
        const platformValue = !!window.Android ? getDeviceData().deviceType : Platform.WebTv;
